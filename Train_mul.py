...
def train(G_type, device, check_gap, dataloader_real, dataloader_monet, G_R2M, G_M2R, D_R, D_M, criterion_GAN, criterion_cycle, optimizer_G,
    ...
    for epoch in range(epoch_num):
        ...
        if epoch % check_gap == 0:
            torch.save(G_M2R.module.state_dict(), f'{G_R2M_SAVE}{G_type}_{epoch}.pth')
    torch.save(G_M2R.module.state_dict(), f'{G_R2M_SAVE}{G_type}_{epoch_num}.pth')
...
G_R2M = torch.nn.DataParallel(G_R2M)
G_M2R = torch.nn.DataParallel(G_M2R)
D_R = torch.nn.DataParallel(D_R)
D_M = torch.nn.DataParallel(D_M)
...
G_type = G_R2M.module.__class__.__name__
batch_size = 12
epoch_num = 500
check_gap = 10
